...
3. Blueprint recipe / Port Block library — authoring/provenance, не runtime
   ...
   этого runtime authoritative (docs/plans/09-01-l1-spatial-foundation-plan L1S.5–6, docs/architecture/blueprints/09-03-port-block-blueprint-architecture).
4. Identity из stable keys и canonical refs: slot_key =
   ...
   никогда не выводится из имён, видимых номеров, координат, порядка (docs/architecture/blueprints/09-03-port-block-blueprint-architecture).
5. Версии ObjectBlueprint/PortBlock неизменяемы; изменения — новая версия
   (docs/plans/09-01-l1-spatial-foundation-plan, docs/architecture/blueprints/09-03-port-block-blueprint-architecture).
6. Blueprint upgrade — additive-only: lock instance, повторный analysis,
   ...
   (docs/plans/09-01-l1-spatial-foundation-plan L1S.6b).
7. Exactly-once canonical writes; после успешного write — refresh-only retry,
   canonical write не повторяется (docs/architecture/presentation/05-presentation Visual wiring, L1S.4c).
8. `UNKNOWN ≠ UNREACHABLE/BLOCKED`; MODEL_ERROR отделён от сетевых вердиктов;
   ...
   результат (docs/00, docs/architecture/presentation/05-presentation).
...
11. RU — язык продукта по умолчанию; локализация не меняет canonical values и
    API payloads (docs/architecture/presentation/05-presentation).
...
- Зависимости: claim в docs/plans/09-01-l1-spatial-foundation-plan уже скорректирован этим проходом
...
- Acceptance: docs/plans/09-01-l1-spatial-foundation-plan явно фиксируют, обновляются ли display aliases
...
benchmark. Профили и бюджеты — [[reviews/10-03-performance-baseline|10.3]].
...
- Acceptance: детерминированный генератор 4 профилей (SMALL/MEDIUM/
  ...
  записан в [[reviews/10-03-performance-baseline|10.3]].
...
- Acceptance: устранены S×T повторных полных обходов и query-per-hop чтения
  ...
  milestone. Any-port trace на MEDIUM укладывается в budget [[reviews/10-03-performance-baseline|10.3]].
- Статус: TODO
...
только после его завершения и фиксации baseline ([[reviews/10-03-performance-baseline|10.3]]).
