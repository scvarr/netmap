...
- [[architecture/l3/01-04-l3|L3 routing model]];
- [[architecture/l3/01-07-policy-routing|Policy Routing]];
- [[architecture/tracing/03-03-l3-trace|L3 resolver]];
- [[architecture/l3/01-05-security-policy|Security Policy]];
- [[architecture/l3/01-06-nat|NAT]];
- [[architecture/tracing/03-02-l2-trace|L2 resolver]];
- L1 physical trace.
...
`PACKET_MARK`, `ROUTING_POLICY` и `ROUTE_DECISION` являются distinct current stage kinds. Их contract определён в [[architecture/l3/01-07-policy-routing|01.7 Policy Routing]].
...
Следующий архитектурный шаг уже не новый network layer. Имеет смысл перейти к [[architecture/graph/02-graph|02. Граф сети]] и определить, как canonical facts компилируются в эффективные graph/projection structures для этих resolver'ов, после чего — к storage/API и инструкции работы системы.
