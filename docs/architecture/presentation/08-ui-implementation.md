...
- ЛКМ по объекту выбирает один объект.
- ЛКМ с протягиванием по свободному месту выполняет прямоугольное
  множественное выделение.
- `Shift + ЛКМ` с протягиванием выполняет прямоугольное выделение только
  объектов/узлов, без кабелей и других связей.
- Средняя кнопка мыши с протягиванием перемещает полотно.
- Колесо мыши изменяет масштаб.
- ЛКМ по пустому месту без протягивания снимает выделение.

Точные модификаторы для добавления и исключения элементов из уже
существующего выделения остаются OPEN.

### Правила попадания в рамку

Узел выбирается рамкой только при полном попадании его отображаемой области
внутрь рамки. Для кабелей и других отображаемых связей простого пересечения
недостаточно: связь выбирается только если вся её отображаемая геометрия
находится внутри рамки. Кабель, выходящий за пределы рамки, не выбирается.
`Shift + рамка` связи вообще не рассматривает. Это предотвращает захват
внешних кабелей при выделении группы оборудования.

### Семантика выделения

Множественное выделение — только текущее состояние UI. Само по себе оно не
создаёт канонические факты, отношения топологии или состояние `SavedMap`, не
создаёт `Location`, `Region`, `MapReference`, `PhysicalObject` и не означает
физического содержимого или группировки. Это только множество элементов, над которым
пользователь затем может выполнить явную команду.
...
Создание реальной стойки или иного `PhysicalObject` требует отдельной явной
операции и собственного семантического контракта.

Это направление может быть удобным способом выбора объектов для будущего
`MapReference`/composite UX, однако `MapReference` не должен архитектурно зависеть
от конкретного способа выбора мышью. Multi-selection не включается
автоматически в B.3 и не смешивается с удобством использования B.4
`MapCableRoute`.
