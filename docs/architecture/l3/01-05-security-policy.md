...
NAT как отдельная packet transformation model определён в [[architecture/l3/01-06-nat|01.6 NAT — packet transformation]]. Полный end-to-end processing order определён концептуально в [[architecture/tracing/03-04-packet-flow-trace|03.4 Packet Flow Trace]].
...
Local mark/fwmark принципиально не входит в `PacketState`. Это transient `LocalProcessingState`, определённый в [[architecture/l3/01-07-policy-routing|01.7 Policy Routing]]. Если platform переносит значение через wire-visible поле вроде DSCP, моделируется именно соответствующее packet field.
