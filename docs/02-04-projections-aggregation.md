...
[[02-05-cache-invalidation|02.5 Cache и invalidation]] фиксирует последнюю core backend policy: cache является только оптимизацией и не должен усложнять первую реализацию до появления measurements.
