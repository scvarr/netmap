    ...
    *,
    ...
    def normalize(value: object, path: str) -> Predicate:
        if not isinstance(value, dict) or not isinstance(value.get("op"), str):
            fail("Security predicate must be an object with a string op", path)
        op = value["op"]
        if op in {"TRUE", "FALSE"}:
            if set(value) != {"op"}:
        ...
        if op in _IP_FIELDS:
            ...
            for index, prefix in enumerate(prefixes):
                if not isinstance(prefix, str):
                    fail("Security predicate prefix must be a string", f"{path}.prefixes[{index}]")
                try:
                    normalized_prefixes.append(str(ip_network(prefix, strict=False)))
                except ValueError as exc:
                    raise error_type(
                        "Security predicate prefix is invalid",
    ...
    if model_error and predicate != normalized:
        raise ModelError(
            "SecurityRule predicate is not canonical",
