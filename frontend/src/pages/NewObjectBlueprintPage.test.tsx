...
const source = () => ({
  ...
  loadPortBlockVersion: vi.fn().mockImplementation(async (_blockId: string, versionId: string) => ({ schema_version: '1.0' as const, port_block_ref: ref('PortBlock', 'pb-1'), name: 'Patch panel', version_ref: ref('PortBlockVersion', versionId), version_number: versionId === 'v-1' ? 1 : 2, ports: [{ local_id: 'p1', kind: 'CONNECTION_POINT' as const, row: 1 as const, column: 1, layout_order: 1 }, { local_id: 'p2', kind: 'NETWORK_PORT' as const, row: 1 as const, column: 2, layout_order: 2 }] })),
