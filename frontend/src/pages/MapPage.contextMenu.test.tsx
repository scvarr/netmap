...
const renderPage = (details: any, disconnect = vi.fn()) => {
  ...
  render(<MemoryRouter initialEntries={["/map?map=map&view=physical"]}><MapPage dataSource={{ loadProjection: vi.fn().mockResolvedValue(document) }} savedMapDataSource={maps} catalogInventoryDataSource={{ loadCatalogInventory: vi.fn().mockResolvedValue({ schema_version: "1.0", equipment: [], cables: [], gaps: [], warnings: [] }) }} physicalObjectDetailsDataSource={{ loadPhysicalObjectDetails: vi.fn().mockResolvedValue(details) }} physicalEndpointConnectionWriteDataSource={{ createPhysicalEndpointConnection: vi.fn(), deleteExternalPhysicalConnection: disconnect }} /></MemoryRouter>);
