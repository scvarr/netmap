...
export function EditObjectBlueprintPage({ dataSource }: { dataSource: ObjectBlueprintDataSource }) {
 ...
 const initial=hydrateBlueprintEditorState(version); if(!initial) return <main className="catalog-page blueprint-editor-page"><div className="breadcrumbs"><Link to="/library/object-blueprints">Библиотека</Link><span>/</span><span>Редактирование</span></div><ViewState kind="empty" message="Structured editor недоступен: эта historical version не содержит authoring recipe. Группы и пары не будут угадываться по slot names."/></main>;
