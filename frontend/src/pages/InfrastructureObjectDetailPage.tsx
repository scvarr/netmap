...
export function InfrastructureObjectDetailPage({
  ...
  useEffect(() => {
    ...
    void catalogInventoryDataSource.loadCatalogInventory().then(
      (document) => {
        if (current) setInventory(document);
      },
      (reason) => {
        if (current) {
          setInventoryError(
  ...
  const node = useMemo(
    () => ({
      ...
      label:
        details?.physical_object.label ??
        t("object.fallbackLabel", { id: physicalObjectId }),
      source_refs: [
        {
      ...
      attributes: {
        ...(details?.physical_object.class
          ? { class: details.physical_object.class }
          : {}),
        ...(details
          ? {
    ...
    <PageShell className="catalog-page object-detail-page">
      ...
      {activeSection === "overview" && details && (
        ...
        >
          ...
          {cable ? (
            ...
            <>
              <p role="alert">
                {t("object.mapLoadError", { error: inventoryError })}
              </p>
              <button
                onClick={() => setInventoryRevision((revision) => revision + 1)}
              >
                {t("action.retry")}
              </button>
            </>
      ...
      {activeSection === "overview" && mapChooser && (
        <section
          ...
          role="dialog"
          aria-modal="true"
          aria-label={t("object.addToMap")}
        >
          <div className="map-dialog__surface">
            <h2>{t("object.addToMap")}</h2>
            {mapChooser.status === "loading" && (
              <p role="status">{t("map.refreshing")}</p>
            )}
            ...
            {mapChooser.status === "ready" &&
              (() => {
                const placed = new Set(
                  inventoryItem?.map_memberships.map(
                    (membership) => membership.map_ref.entity_id,
                  ),
                );
                ...
                if (mapChooser.maps.length === 0)
                  return <p>{t("object.mapsEmpty")}</p>;
                if (available.length === 0)
                  return <p>{t("object.alreadyOnMaps")}</p>;
                return (
                  <ul>
                    {available.map((map) => (
                      <li key={map.map_ref.entity_id}>
                        <Link
                          to={addMapLink(
                            map.map_ref.entity_id,
                            physicalObjectId,
                          )}
                        >
                          {map.name}
            ...
            <button type="button" onClick={() => setMapChooser(null)}>
              {t("action.close")}
            </button>
          </div>
        </section>
      ...
      {activeSection === "physical" && <section className="detail-section detail-section--operations">
        <PhysicalObjectDetailsSection
          ...
          dataSource={
            physicalObjectDetailsDataSource ?? {
              loadPhysicalObjectDetails: () =>
                Promise.reject(
                  new Error(t("inspector.objectDetailsLoadFailed")),
                ),
            }
          }
