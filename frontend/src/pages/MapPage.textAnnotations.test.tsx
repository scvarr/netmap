...
vi.mock('../components/TopologyCanvas', () => ({ TopologyCanvas: (props: any) => <>
  <button onClick={() => props.annotationMode?.onAnnotationPlace?.({ x: 12, y: 34 })}>place text</button>
  <button onClick={() => props.annotationMode?.onMoveAnnotation?.('annotation-a', { x: 70, y: -30 })}>drag text</button>
  <button onClick={() => props.annotationMode?.onAnnotationSelect?.('annotation-a')}>select annotation</button>
  <output data-testid="annotation-preview">{JSON.stringify(props.annotationMode?.previewAnnotation ?? null)}</output>
</> }));
...
const map = { active_variant_ref: { entity_type: 'MapPresentationVariant' as const, entity_id: 'variant-a' }, variants: [{ variant_ref: { entity_type: 'MapPresentationVariant' as const, entity_id: 'variant-a' }, name: 'Основной' }], map_ref: { entity_type: 'SavedMap' as const, entity_id: mapId }, name: 'A', created_at: '', updated_at: '', placements: [], cable_routes: [], text_annotations: [annotation] };
...
const enterAnnotationMode = async () => { fireEvent.click(await screen.findByRole('button', { name: 'Инструменты' })); fireEvent.click(screen.getByRole('button', { name: 'Текст' })); };
...
describe('MapPage text annotations', () => {
  it('keeps a new annotation local through placement and editing, then creates exactly once and reloads authoritatively', async () => {
    ...
    renderPage(maps); await enterAnnotationMode();
  ...
  it('cancels an unacknowledged annotation without a write and validates blank text', async () => {
    ...
    renderPage(maps); await enterAnnotationMode(); fireEvent.click(screen.getByRole('button', { name: 'Добавить текст' })); fireEvent.click(screen.getByText('place text'));
  ...
  it('free-drags an existing annotation, replaces it once, and retries only reload after acknowledgement', async () => {
    ...
    renderPage(maps); await enterAnnotationMode(); fireEvent.click(screen.getByRole('button', { name: 'Existing' })); fireEvent.click(screen.getByRole('button', { name: 'Изменить текст' })); fireEvent.click(screen.getByText('drag text'));
  ...
  it('confirms deletion and retries only refresh after one acknowledged DELETE', async () => {
    ...
    renderPage(maps); await enterAnnotationMode(); fireEvent.click(screen.getByRole('button', { name: 'Existing' })); fireEvent.click(screen.getByRole('button', { name: 'Удалить текст' })); expect(deleteTextAnnotation).not.toHaveBeenCalled(); maps.loadMap.mockRejectedValueOnce(new Error('refresh'));
