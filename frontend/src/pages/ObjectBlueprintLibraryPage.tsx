...
export function ObjectBlueprintLibraryPage({ dataSource }: { dataSource: ObjectBlueprintDataSource }) {
  ...
  return <main className="catalog-page blueprint-library-page">
    ...
    <section className="blueprint-library-page__surface">
      ...
      {document && document.blueprints.length > 0 && <div className="blueprint-library-table-wrap"><table className="blueprint-library-table"><thead><tr><th scope="col">{t('blueprint.library.name')}</th><th scope="col">{t('blueprint.library.objectType')}</th><th scope="col">{t('blueprint.library.version')}</th><th scope="col">{t('blueprint.library.proportions')}</th><th scope="col">{t('blueprint.library.connectionPoints')}</th><th scope="col">{t('blueprint.library.networkPorts')}</th><th scope="col">{t('blueprint.library.internalLinks')}</th><th scope="col">{t('blueprint.library.actions')}</th></tr></thead><tbody>{document.blueprints.map((blueprint) => {
        const version = versions[blueprint.version_ref.entity_id];
        const connectionPoints = version ? version.slots.filter((slot) => slot.kind === 'CONNECTION_POINT').length : '—';
        const networkPorts = version ? version.slots.filter((slot) => slot.kind === 'NETWORK_PORT').length : '—';
        return <tr key={blueprint.blueprint_ref.entity_id}><th scope="row">{blueprint.name}</th><td>{blueprint.default_physical_object_class || t('blueprint.library.notSpecified')}</td><td>{t('blueprint.library.versionSummary', { version: blueprint.version_number, count: blueprint.version_count })}</td><td className="blueprint-library-table__numeric">{blueprint.body.width} × {blueprint.body.height}</td><td className="blueprint-library-table__numeric">{connectionPoints}</td><td className="blueprint-library-table__numeric">{networkPorts}</td><td className="blueprint-library-table__numeric">{blueprint.internal_link_count}</td><td><div className="blueprint-library-table__actions"><Link className="blueprint-icon-action" aria-label={t('blueprint.library.createObject')} title={t('blueprint.library.createObject')} to={`/infrastructure/objects/new?blueprint=${encodeURIComponent(blueprint.blueprint_ref.entity_id)}&version=${encodeURIComponent(blueprint.version_ref.entity_id)}`}>+</Link><Link className="blueprint-icon-action" aria-label={t('blueprint.library.edit')} title={t('blueprint.library.edit')} to={`/library/object-blueprints/${blueprint.blueprint_ref.entity_id}/versions/${blueprint.version_ref.entity_id}/edit`}>✎</Link><button type="button" className="blueprint-icon-action blueprint-icon-action--danger" aria-label={t('blueprint.library.delete')} title={t('blueprint.library.delete')} onClick={() => void remove(blueprint.blueprint_ref.entity_id, blueprint.name)}>×</button></div></td></tr>;
      })}</tbody></table></div>}
    </section>
  </main>;
}
