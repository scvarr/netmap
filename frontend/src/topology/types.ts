...
export interface BlueprintPresentation {
  ...
  slots: Array<{ slot_key: string; display_name: string; kind: 'CONNECTION_POINT' | 'NETWORK_PORT'; face?: 'FRONT' | 'REAR'; rendered_position: { x: number; y: number }; external_attachment: { x: number; y: number; side: 'LEFT' | 'RIGHT' | 'TOP' | 'BOTTOM' }; connection_point_id: string; network_interface_id?: string | null }>;
}
export interface PhysicalEndpointPair { from_connection_point_id: string; from_member_index: number; to_connection_point_id: string; to_member_index: number; connection_id: string; connection_member_id: string; cable_ref?: ProjectionSourceRef; cable_display_name?: string; }
...
export interface TopologyProjectionRequest {
  ...
  include_cable_continuations?: boolean;
