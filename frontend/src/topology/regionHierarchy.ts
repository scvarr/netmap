...
const pointIsStrictlyInside = (point: MapRegionPoint, polygon: readonly MapRegionPoint[]) => {
  ...
  for (let index = 0, previous = polygon.length - 1; index < polygon.length; previous = index++) {
    ...
    const onSegment = cross === 0 && point.x >= Math.min(start.x, end.x) && point.x <= Math.max(start.x, end.x) && point.y >= Math.min(start.y, end.y) && point.y <= Math.max(start.y, end.y);
...
export const deriveRegionHierarchy = (regions: readonly MapRegion[]): RegionHierarchyNode[] => {
  ...
  const valid = all.filter((region) => hasFinitePoints(region.points) && absolutePolygonArea(region.points) > 0);
  ...
  for (const region of valid) {
    const candidates = valid.filter((outer) => outer.region_ref.entity_id !== region.region_ref.entity_id && (areas.get(outer.region_ref.entity_id) ?? 0) > (areas.get(region.region_ref.entity_id) ?? 0) && region.points.every((point) => pointIsStrictlyInside(point, outer.points))).sort((left, right) => (areas.get(left.region_ref.entity_id)! - areas.get(right.region_ref.entity_id)!) || compareRegions(left, right));
    if (candidates.length > 0) parentById.set(region.region_ref.entity_id, candidates[0].region_ref.entity_id);
  }
