...
const document = (): any => ({
  ...
  cables: [
    { cable_ref: ref('Cable', 'cable'), connection_ref: ref('Connection', 'connection'), label: 'C-001', resolution: 'RESOLVED', endpoint_a: { remote_physical_object_ref: ref('PhysicalObject', 'left'), remote_physical_object_label: 'Left', remote_connection_point_ref: ref('ConnectionPoint', 'lp'), remote_connection_point_label: 'L1', evidence_refs: [ref('Connection', 'connection')] }, endpoint_b: { remote_physical_object_ref: ref('PhysicalObject', 'right'), remote_physical_object_label: 'Right', remote_connection_point_ref: ref('ConnectionPoint', 'rp'), remote_connection_point_label: 'R1', evidence_refs: [ref('Connection', 'connection')] }, gaps: [], warnings: [] },
    { cable_ref: ref('Cable', 'cable-2'), connection_ref: ref('Connection', 'connection-2'), label: 'C-002', label_source: 'TECHNICAL_FALLBACK', resolution: 'RESOLVED', endpoint_a: { remote_physical_object_ref: ref('PhysicalObject', 'left'), remote_physical_object_label: 'Left', remote_connection_point_ref: ref('ConnectionPoint', 'lp'), remote_connection_point_label: 'L1', evidence_refs: [ref('Connection', 'connection-2')] }, endpoint_b: { remote_physical_object_ref: ref('PhysicalObject', 'right'), remote_physical_object_label: 'Right', remote_connection_point_ref: ref('ConnectionPoint', 'rp'), remote_connection_point_label: 'R1', evidence_refs: [ref('Connection', 'connection-2')] }, gaps: [], warnings: [] },
  ], gaps: [], warnings: [],
});
...
describe('ApiCatalogInventoryDataSource', () => {
  it('parses equipment, map memberships, optional occupancy, and cable resolutions', () => {
    ...
    expect(parsed.cables[1].connection_ref.entity_type).toBe('Connection');
  ...
  it('rejects malformed canonical refs and invalid cable relation shapes', () => {
    ...
    invalidConnection.cables[1].connection_ref = ref('PhysicalObject', 'not-a-connection');
    expect(() => parseCatalogInventoryDocument(invalidConnection)).toThrow(/connection_ref/);
