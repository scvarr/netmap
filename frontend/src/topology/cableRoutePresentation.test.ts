...
const cable = (id: string, label = 'same label') => ({
  ...
  kind: 'CABLE',
  label,
  source_refs: [{ ref_type: 'CANONICAL_FACT' as const, entity_type: 'Cable', entity_id: id }],
  attributes: {},
});
const route = (id: string, waypoints = [{ x: 12, y: -3 }]) => ({
  cable_ref: { ref_type: 'CANONICAL_FACT' as const, entity_type: 'Cable', entity_id: id },
  view: 'L1/PHYSICAL_OBJECT' as const,
  waypoints,
});

describe('authoritative collapsed cable route lookup', () => {
  it('matches only the exact canonical Cable identity', () => {
