...
describe('Blueprint editor group expansion', () => {
  ...
  it('hydrates persisted stable group identity without rebuilding it', () => {
    const state = hydrateBlueprintEditorState({
      ...
      authoring_recipe: { endpoint_groups: [{ group_id: 'persisted-id', key_prefix: 'persisted-stable-key', display_prefix: 'A', kind: 'CONNECTION_POINT', side: 'LEFT', count: 1, starting_number: 1, placement_offset: 0, placement_span: 1 }], pair_recipes: [] },
    });
