import type { BlueprintFace, BlueprintInstanceNaming, BlueprintInternalLink, BlueprintPortBlockPlacement, CreateObjectBlueprintRequest, ObjectBlueprintVersionDocument } from '../topology/objectBlueprintTypes';
import type { PortBlockDataSource, PortBlockVersionDocument } from '../topology/portBlockTypes';

export interface BlueprintBlockInstance { instanceKey: string; portBlockRef: string; portBlockVersionRef: string; face?: BlueprintFace; placement?: BlueprintPortBlockPlacement; portBlockName: string; versionNumber: number; ports: PortBlockVersionDocument['ports']; resolvedSlotKeys: Record<string, string>; naming: BlueprintInstanceNaming; }
...
export type BlueprintValidationError = 'nameRequired' | 'dimensionsPositive' | 'colorFormat' | 'duplicateInstanceKey' | 'missingPortBlock' | 'invalidNaming' | 'individualSelfLink' | 'individualMissingPort' | 'duplicateIndividualLink';
export const defaultNaming = (ports: BlueprintBlockInstance['ports']): BlueprintInstanceNaming => ({ prefix: '', starting_number: 1, mode: ports.some((port) => port.row === 2) ? 'SEQUENTIAL' : 'SINGLE', overrides: {} });
export const resolvedNames = (item: Pick<BlueprintBlockInstance, 'ports' | 'naming'>): Record<string, string> => {
  const byRow = [1, 2].map((row) => item.ports.filter((port) => port.row === row).sort((a, b) => a.layout_order - b.layout_order));
  const names: Record<string, string> = {};
  byRow.forEach((ports, rowIndex) => ports.forEach((port, index) => {
    const { mode, starting_number: start, prefix, overrides } = item.naming;
    const number = mode === 'SINGLE' ? start + index : mode === 'SEQUENTIAL' ? start + rowIndex * ports.length + index : mode === 'ODD_EVEN' ? start + index * 2 + rowIndex : start + index * 2 + (rowIndex === 0 ? 1 : 0);
    names[port.local_id] = overrides[port.local_id] ?? `${prefix}${number}`;
  }));
  return names;
};
...
export const slotsForInstance = (item: BlueprintBlockInstance) => { const names = resolvedNames(item); return item.ports.flatMap((port) => item.resolvedSlotKeys?.[port.local_id] ? [{ key:item.resolvedSlotKeys[port.local_id], label: `${item.portBlockName ?? item.portBlockVersionRef} · ${names[port.local_id]}`, displayName: names[port.local_id], kind: port.kind }] : []); };
...
export const hydrateBlueprintEditorState = async (version: ObjectBlueprintVersionDocument, source: PortBlockDataSource): Promise<BlueprintEditorState | null> => {
  ...
  const instances = await Promise.all(version.composition.instances.map(async (item, index, sourceItems) => {
    ...
    const instance = { instanceKey:item.instance_key, portBlockRef:item.port_block_ref.entity_id, portBlockVersionRef:item.port_block_version_ref.entity_id, face:item.face ?? 'FRONT' as BlueprintFace, placement: item.placement ? clampPlacement(item.placement) : fallbackPlacement(faceLocalIndex(sourceItems, index)), portBlockName:exact.name, versionNumber:exact.version_number, ports:exact.ports, naming:item.naming, resolvedSlotKeys:{} };
...
export const generateBlueprint = (state: BlueprintEditorState) => {
  ...
  if (state.instances.some((item) => { const rows = new Set(item.ports.map((port) => port.row)); const names = resolvedNames(item); return !Number.isInteger(item.naming.starting_number) || item.naming.starting_number < 0 || (item.naming.mode === 'SINGLE' ? rows.size !== 1 : rows.size !== 2) || Object.keys(item.naming.overrides).some((key) => !item.ports.some((port) => port.local_id === key)) || Object.values(names).some((name) => !name.trim() || name.length > 255); })) errors.push('invalidNaming');
  const source = state.instances.flatMap(slotsForInstance); const slots = source.map((slot) => ({ key:slot.key, display_name:slot.displayName, kind:slot.kind }));
