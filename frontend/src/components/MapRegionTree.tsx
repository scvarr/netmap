...
export function MapRegionTree({ regions, selectedRegionId, onSelect }: { regions: readonly MapRegion[]; selectedRegionId: string | null; onSelect: (regionId: string) => void }) {
  ...
  const renderNode = (node: RegionHierarchyNode, depth: number): ReactNode => {
    ...
    return <li key={id}><div className="map-region-tree__row" style={{ paddingInlineStart: `${depth * 16}px` }}>
      ...
      <button type="button" className="map-region-tree__select" aria-pressed={selectedRegionId === id} onClick={() => onSelect(id)}>{node.region.label}</button>
