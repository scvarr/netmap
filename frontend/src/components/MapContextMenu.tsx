...
interface MapContextMenuProps {
  ...
  onEditBlueprintSize?: (id: string) => void; onCopyBlueprintSize?: (width: number) => void;
  onApplyCopiedBlueprintSize?: (id: string, width: number) => void; onApplyBlueprintSizeToSameBlueprint?: (id: string) => void;
  onRenameCable?: (id: string, label: string) => void; onEditRoute?: (id: string) => void; onResetRoute?: (id: string) => void; onDeleteCable?: (id: string, label: string) => void;
  onConnectFromPort?: (objectId: string, connectionPointId: string) => void; onDisconnect?: (connectionId: string, label: string) => void;
  onBulkConnect?: (objectId: string) => void;
}
export function MapContextMenu({ target, onClose, ...actions }: MapContextMenuProps) {
  ...
  return <div ref={ref} className="map-context-menu" role="menu" style={{ left: position.x, top: position.y }}>
    ...
    {target.kind === "object" && actions.onBulkConnect && <button role="menuitem" onClick={() => run(() => actions.onBulkConnect?.(target.id))}>{t("map.bulk.action")}</button>}
  </div>;
}
