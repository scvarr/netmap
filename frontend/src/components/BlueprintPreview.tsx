...
export function BlueprintPreview({ body, slots, internalLinks = [], label = 'Схематический preview шаблона', style }: BlueprintPreviewProps) {
  const points = new Map(slots.map((slot) => [slot.key, pointFor(slot, body)]));
  const padding = Math.max(body.width, body.height) * .08;
  return <svg className="blueprint-preview" style={style} viewBox={`${-padding} ${-padding} ${body.width + padding * 2} ${body.height + padding * 2}`} role="img" aria-label={label} preserveAspectRatio="xMidYMid meet" data-ratio={body.width / body.height}>
