...
def slot(local_id: str, kind: str = "CONNECTION_POINT") -> dict:
    return {"local_id": local_id, "kind": kind, "row": 1, "column": 1, "layout_order": 1}
...
def create_blueprint(slots: list[dict], links: list[dict] | None = None, *, instance_key: str = "instance", face: str = "FRONT", name: str = "Blueprint", port_block: tuple[str, str] | None = None, **extra: object) -> tuple[str, str]:
    ...
    instance_placement = extra.pop("placement", placement)
    response = client.post("/v1/library/object-blueprints", json={
        ...
        "composition": {"instances": [{"instance_key": instance_key, "port_block_version_ref": {"ref_type": "LIBRARY_RECORD", "entity_type": "PortBlockVersion", "entity_id": port_block_version_id}, "face": face, "placement": instance_placement, "naming": {"prefix": "", "starting_number": 1, "mode": "SINGLE", "overrides": {item["local_id"]: item["local_id"] for item in slots}}}]},
...
def test_composition_expands_exact_ports_and_reads_provenance():
    ...
    assert body["composition"] == {"instances": [{"instance_key": "K", "port_block_ref": {"ref_type": "LIBRARY_RECORD", "entity_type": "PortBlock", "entity_id": block_id}, "port_block_version_ref": {"ref_type": "LIBRARY_RECORD", "entity_type": "PortBlockVersion", "entity_id": block_version_id}, "face": "FRONT", "placement": placement, "naming": {"prefix": "", "starting_number": 1, "mode": "SINGLE", "overrides": {"p1": "p1", "p2": "p2"}}}]}
...
def test_same_exact_block_twice_has_distinct_final_slots_and_duplicate_instance_key_is_rejected():
    ...
    payload = {"name": "Twice", "body": {"kind": "RECTANGLE", "width": 1, "height": 1}, "composition": {"instances": [{"instance_key": "left", "port_block_version_ref": ref, "face": "FRONT", "placement": placement, "naming": naming}, {"instance_key": "right", "port_block_version_ref": ref, "face": "REAR", "placement": placement, "naming": naming}]}, "internal_links": []}
    created = client.post("/v1/library/object-blueprints", json=payload)
    ...
    duplicate = client.post("/v1/library/object-blueprints", json={**payload, "composition": {"instances": [{"instance_key": "same", "port_block_version_ref": ref, "face": "FRONT", "placement": placement, "naming": naming}, {"instance_key": "same", "port_block_version_ref": ref, "face": "FRONT", "placement": placement, "naming": naming}]}})
...
def test_explicit_cross_block_links_reject_self_missing_and_unordered_duplicates():
    ...
    base = {"name": "Links", "body": {"kind": "RECTANGLE", "width": 1, "height": 1}, "composition": {"instances": [{"instance_key": "left", "port_block_version_ref": ref, "face": "FRONT", "placement": placement, "naming": naming}, {"instance_key": "right", "port_block_version_ref": ref, "face": "REAR", "placement": placement, "naming": naming}]}}
...
def test_placement_round_trips_and_cannot_change_composed_slot_identity():
    block_id, block_version_id = create_port_block([slot("p1")])
    blueprint_id, version_id = create_blueprint([slot("p1")], instance_key="stable", port_block=(block_id, block_version_id), placement={"x": .4, "y": .5, "width": .3, "height": .2})
    detail = client.get(f"/v1/library/object-blueprints/{blueprint_id}/versions/{version_id}").json()
    assert detail["composition"]["instances"][0]["placement"] == {"x": .4, "y": .5, "width": .3, "height": .2}
    assert detail["slots"][0]["key"] == composed_slot_key("stable", "p1")
    created = client.post(f"/v1/library/object-blueprints/{blueprint_id}/versions", json={"body": {"kind": "RECTANGLE", "width": 100, "height": 40}, "composition": {"instances": [{"instance_key": "stable", "port_block_version_ref": {"ref_type": "LIBRARY_RECORD", "entity_type": "PortBlockVersion", "entity_id": block_version_id}, "face": "FRONT", "placement": {"x": .05, "y": .05, "width": .8, "height": .7}, "naming": naming}]}, "internal_links": []})
    assert created.status_code == 201
    next_detail = client.get(f"/v1/library/object-blueprints/{blueprint_id}/versions/{created.json()['version_ref']['entity_id']}").json()
    assert next_detail["slots"][0]["key"] == detail["slots"][0]["key"]


def test_out_of_bounds_placement_is_rejected():
    _, block_version_id = create_port_block([slot("p1")])
    payload = {"name": "Invalid placement", "body": {"kind": "RECTANGLE", "width": 1, "height": 1}, "composition": {"instances": [{"instance_key": "one", "port_block_version_ref": {"ref_type": "LIBRARY_RECORD", "entity_type": "PortBlockVersion", "entity_id": block_version_id}, "face": "FRONT", "placement": {"x": .8, "y": 0, "width": .3, "height": .2}}]}, "internal_links": []}
