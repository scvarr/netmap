...
def generate(profile_name: str, seed: int) -> dict[str, object]:
    ...
    with SessionLocal() as session:
        result: dict[str, object] = {"physical_objects": session.scalar(select(func.count()).select_from(PhysicalObject)) or 0, "connection_points": session.scalar(select(func.count()).select_from(ConnectionPoint)) or 0, "network_interfaces": session.scalar(select(func.count()).select_from(NetworkInterface)) or 0, "connections": session.scalar(select(func.count()).select_from(Connection)) or 0, "saved_maps": session.scalar(select(func.count()).select_from(SavedMap)) or 0, "map_memberships": session.scalar(select(func.count()).select_from(MapPlacement)) or 0,
