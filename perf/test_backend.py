...
def test_backend_baseline(perf_context, request):
    ...
    cases = {
        "projection_l1": ("projection_l1_scoped_interstitial", "POST", "/v1/topology/projection", _projection("L1", perf_context["object_ids"], True)),
        "projection_l2": ("projection_l2_unscoped", "POST", "/v1/topology/projection", _projection("L2")),
        "saved_map": ("saved_map", "GET", f"/v1/maps/{perf_context['map_id']}", None),
        "catalog_inventory": ("catalog_inventory", "GET", "/v1/catalog/inventory", None),
        "physical_object": ("physical_object", "GET", f"/v1/topology/physical-objects/{perf_context['object_ids'][0]}", None),
        "trace_specific_port": ("trace_specific_port", "POST", "/v1/traces/physical-objects/l1", {"from_physical_object_id": perf_context["trace_source"], "to_physical_object_id": perf_context["trace_target"], "from_connection_point_id": perf_context["point_id"]}),
        "trace_any_port": ("trace_any_port", "POST", "/v1/traces/physical-objects/l1", {"from_physical_object_id": perf_context["trace_source"], "to_physical_object_id": perf_context["trace_target"]}),
    }
    results = []
    selected = request.config.getoption("--perf-case")
    assert selected, "--perf-case is required for perf/test_backend.py"
    metric_name, method, url, body = cases[selected]
    with httpx.Client(base_url=base_url, timeout=120) as client:
        ...
        results.append({"metric": metric_name, "http_e2e_ms": summary["median_ms"], "sql_query_count": summary["query_count_median"], "response_bytes": summary["response_bytes_median"], "runs": summary["runs"], "p50_ms": summary["p50_ms"], "p95_ms": summary["p95_ms"]})
        # Resolver-only has its own direct boundary; HTTP e2e remains separately measured above.
        if selected == "projection_l1":
            query = TopologyProjectionRequest.model_validate(_projection("L1", perf_context["object_ids"], True)); resolver_runs = []
            for _ in range(_runs(perf_context["mode"])):
                with SessionLocal() as session:
            ...
            resolver = summarize(resolver_runs, perf_context["mode"]); http_projection = results[0]
